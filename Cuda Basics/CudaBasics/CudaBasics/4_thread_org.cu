...
int main() {
	...
	// For:dkernel_2D
	dim3 threadPerBlock(N, M, 1); // 2D block of threads of 5*6 = 30 threads // N in X direction and M in Y direction
	unsigned* matrix, * hmatrix;

	cudaMalloc(&matrix, N * M * sizeof(unsigned));
	hmatrix = (unsigned*)malloc(N * M * sizeof(unsigned));

	dkernel_2D << <1, threadPerBlock >> > (matrix);
...
/*
A[2][5]  // row 2, column 5
  	0 | 1 | 2 | 3 | 4 |
0 | 00, 01, 02, 03, 04
1 | 10, 11, 12, 13, 14

dim3 threadsPerBlock(5, 2); // column 5, row 2 
col = threadIdx.x; // 2 // x index for column
row = threadIdx.y; // 1 // y index for row
A[row][col] = a[1][2]
